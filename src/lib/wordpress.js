...
// ===============================
// FUNCIONES PARA BLOG (NUEVO)
// ===============================
export async function getBlogsPaginados({ page = 1, itemsPerPage = 12, categoria = null } = {}) {
  console.log(`📝 Obteniendo página ${page} con ${itemsPerPage} blogs${categoria ? ` (categoría: ${categoria})` : ''}...`);
  
  // Obtener más blogs de los necesarios para poder filtrar y paginar
  const itemsToLoad = Math.max(100, page * itemsPerPage * 3);
  
  const QUERY = gql`
    query GetAllBlogsPaginated($first: Int!) {
      posts(
        first: $first
        where: {
          orderby: { field: DATE, order: DESC }
  ...
  try {
    console.log(`📄 [BLOG] Página ${page}: Cargando ${itemsToLoad} blogs...`);
    
    const variables = { first: itemsToLoad };
    const rawData = await request(WORDPRESS_GRAPHQL_URL, QUERY, variables);
    
    let allNodes = rawData?.posts?.nodes || [];
    const pageInfo = rawData?.posts?.pageInfo;
    
    console.log(`✅ Obtenidos ${allNodes.length} blogs totales`);
    
    if (!Array.isArray(allNodes) || allNodes.length === 0) {
      console.warn('⚠️ No se obtuvieron blogs');
      return {
        blogs: [],
        pageInfo: {
          hasNext: false,
    ...
    // FILTRAR POR CATEGORÍA si se especifica una
    if (categoria && categoria !== 'all') {
      allNodes = allNodes.filter(blog => {
        const categoriasBlog = blog.categories?.nodes || [];
        return categoriasBlog.some(cat => cat.slug === categoria);
      });
      console.log(`🔍 Después del filtro por categoría '${categoria}': ${allNodes.length} blogs`);
    }
    ...
    console.log(`🔍 Página ${page}: mostrando ${pageNodes.length} blogs [${startIndex}:${endIndex}]`);
    
    const blogsProcesados = pageNodes.map((b) => {
      const imgUrl = processImageURL(b.featuredImage?.node?.sourceUrl) || '/images/placeholder-news.jpg';
      const alt = b.featuredImage?.node?.altText || b.seo?.title || b.title;
      
      let cleanExcerpt = '';
      if (b.excerpt) {
        cleanExcerpt = b.excerpt.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
        cleanExcerpt = cleanExcerpt.substring(0, 160) + (cleanExcerpt.length > 160 ? '...' : '');
      } else if (b.content) {
        cleanExcerpt = b.content.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
        cleanExcerpt = cleanExcerpt.substring(0, 160) + (cleanExcerpt.length > 160 ? '...' : '');
      } else {
        cleanExcerpt = b.seo?.metaDesc || `Lee el blog "${b.title || 'desconocido'}".`;
      }
      ...
      return {
        ...
        author: b.author?.node || null,
        seo: b.seo?.title ? b.seo : {
          title: `${b.title || 'Blog'} | VinylStation Blog`,
          metaDesc: cleanExcerpt
        }
      };
    });
    ...
    return {
      blogs: blogsProcesados,
      pageInfo: {
        hasNext: hasMoreItems || pageInfo?.hasNextPage,
        hasPrevious: page > 1,
        total: totalFiltered,
        totalBlogs: totalFiltered,
        totalPages,
        currentPage: page,
        itemsPerPage
      }
    ...
    console.error('❌ Error obteniendo blogs:', error.message);
    if (error.response?.errors) {
      console.error('GraphQL Errors:', JSON.stringify(error.response.errors, null, 2));
    }
...
export async function getBlogs({ limit = 20000 } = {}) {
  console.log(`📝 Obteniendo HASTA ${limit} blogs con paginación...`);
  
  const QUERY = gql`
    query GetPaginatedBlogs($first: Int!, $after: String) {
  ...
  try {
    while (hasNextPage && allBlogs.length < limit) {
      const remainingLimit = limit - allBlogs.length;
      const itemsToFetch = Math.min(ITEMS_PER_PAGE, remainingLimit);
      if (itemsToFetch <= 0) break;
      
      console.log(`📄 Petición ${pageCount} (Blogs): Obteniendo hasta ${itemsToFetch}.`);
      const variables = { first: itemsToFetch, after: cursor };
      const data = await request(WORDPRESS_GRAPHQL_URL, QUERY, variables);
      ...
      if (!Array.isArray(nodes) || !pageInfo) {
        console.warn(`⚠️ getBlogs (p${pageCount}): 'nodes' o 'pageInfo' inválidos. Deteniendo.`);
        break;
      }
      ...
      console.log(`  👍 Blogs p${pageCount}: ${nodes.length} recibidos. hasNextPage: ${hasNextPage}. Total: ${allBlogs.length}`);
      pageCount++;
      
      if (pageCount > 250) {
        console.warn('⚠️ Límite de páginas (250) alcanzado en getBlogs.');
        break;
      }
    }
    
    const blogsProcessed = allBlogs.map((b) => {
      const imgUrl = processImageURL(b.featuredImage?.node?.sourceUrl) || '/images/placeholder-news.jpg';
      const alt = b.featuredImage?.node?.altText || b.seo?.title || b.title;
    ...
    console.log(`✅ TOTAL FINAL (Blogs): ${blogsProcessed.length} procesados.`);
    
    return {
      blogs: blogsProcessed,
      pageInfo: {
        hasNextPage: hasNextPage && allBlogs.length < limit,
        endCursor: cursor,
        total: blogsProcessed.length
      }
    };
    
  } catch (error) {
    console.error('❌ Error en getBlogs:', error.message);
    if (error.response?.errors) {
      console.error('GraphQL Errors (getBlogs):', JSON.stringify(error.response.errors, null, 2));
    }
    return { blogs: [], pageInfo: { hasNextPage: false, endCursor: null, total: 0 } };
  }
}

export async function getBlogBySlug(slug) {
  console.log(`📝 Obteniendo blog: ${slug}`);
  ...
  try {
    const { post } = await request(WORDPRESS_GRAPHQL_URL, QUERY, { slug });
    
    if (!post) {
        console.warn(`⚠️ No se encontró blog con slug: ${slug}`);
        return null;
    }

    const imgUrl = processImageURL(post.featuredImage?.node?.sourceUrl) || '/images/placeholder-news.jpg';
    const alt = post.featuredImage?.node?.altText || post.seo?.title || post.title;
    
    let cleanExcerpt = '';
    if (post.excerpt) {
        cleanExcerpt = post.excerpt.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
        cleanExcerpt = cleanExcerpt.substring(0, 160) + (cleanExcerpt.length > 160 ? '...' : '');
    } else if (post.content) {
        cleanExcerpt = post.content.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
        cleanExcerpt = cleanExcerpt.substring(0, 160) + (cleanExcerpt.length > 160 ? '...' : '');
    } else {
        cleanExcerpt = post.seo?.metaDesc || `Lee el blog "${post.title || 'desconocido'}".`;
    }
    ...
    return {
      ...
      seo: post.seo?.title ? post.seo : {
        title: `${post.title || 'Blog'} | VinylStation Blog`,
        metaDesc: cleanExcerpt
      }
    };
    
  } catch (error) {
    console.error(`❌ Error en getBlogBySlug(${slug}):`, error.message);
    if (error.response?.errors) {
        console.error('GraphQL Errors (getBlogBySlug):', JSON.stringify(error.response.errors, null, 2));
    }
    return null;
  }
}
...
export async function getCategoriasBlog() {
  console.log('🏷️ Obteniendo categorías de blog...');
  
  const QUERY = gql`
    query GetCategoriasBlog {
      categories(first: 100, where: { hideEmpty: true, orderby: COUNT, order: DESC }) {
        nodes {
          id
          name
          slug
  ...
  try {
    const { categories } = await request(WORDPRESS_GRAPHQL_URL, QUERY);
    
    const nodes = categories?.nodes || [];
    console.log(`✅ Obtenidas ${nodes.length} categorías de blog.`);
    return { categorias: nodes };
    
  } catch (error) {
    console.error('❌ Error en getCategoriasBlog:', error.message);
    if (error.response?.errors) {
        console.error('GraphQL Errors (getCategoriasBlog):', JSON.stringify(error.response.errors, null, 2));
    }
    return { categorias: [] };
  }
}

export async function getTodosLosBlogs() {
  console.log('🔍 Cargando TODOS los blogs para paginación estática...');
  ...
  try {
    while (hasNextPage) {
      pageCount++;
      console.log(`📄 Cargando lote ${pageCount} de blogs...`);
      ...
      const rawData = await request(WORDPRESS_GRAPHQL_URL, QUERY_ALL, variables);
      const nodes = rawData?.posts?.nodes || [];
      const pageInfo = rawData?.posts?.pageInfo;
      
      if (nodes.length > 0) {
        allBlogs = allBlogs.concat(nodes);
        console.log(`   ✅ Lote ${pageCount}: ${nodes.length} blogs cargados (total: ${allBlogs.length})`);
      }
    ...
    console.log(`✅ Total de blogs cargados: ${allBlogs.length}`);
    
    // Procesar todos los blogs
    const blogsProcesados = allBlogs.map((b) => {
      const imgUrl = processImageURL(b.featuredImage?.node?.sourceUrl) || '/images/placeholder-news.jpg';
      const alt = b.featuredImage?.node?.altText || b.seo?.title || b.title;
    ...
    return {
      blogs: blogsProcesados,
      total: blogsProcesados.length
    };
    
  } catch (error) {
    console.error('❌ Error cargando todos los blogs:', error.message);
    if (error.response?.errors) {
      console.error('GraphQL Errors:', JSON.stringify(error.response.errors, null, 2));
    }
    return {
